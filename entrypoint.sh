...
remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; cleanup() { log 'Removing workspace...'; rm -rf \"\$HOME/apps/${WORKSPACE}\" ; } ; log 'Creating workspace directory...' ; mkdir -p \"\$HOME/apps/${WORKSPACE}\" ; chmod -R 755 \"\$HOME/apps/${WORKSPACE}\"; trap cleanup EXIT ; log 'Unpacking workspace...' ; tar -C \"\$HOME/apps/${WORKSPACE}\" -xjv ; log 'Launching docker-compose...' ; cd \"\$HOME/apps/${WORKSPACE}\" ; $ENV_ARGS docker-compose -f \"$DOCKER_COMPOSE_FILENAME\" -p \"$DOCKER_COMPOSE_PREFIX\" up -d --remove-orphans --build"
